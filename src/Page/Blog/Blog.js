    ...
    return (
        <div className='w-4/6 mx-auto text-left py-10'>
            ...
            <p><span className='text-bold text-green-600'>Ans:</span> There are four main types of state you need to properly manage in your React apps. Local state, Global state, Server state, URL state.
                <br />
                <span className='font-bold'>Local state</span> is perhaps the easiest kind of state to manage in React, considering there are so many tools built into the core React library for managing it. useState is the first tool you should reach for to manage state in your components. It can take accept any valid data value, including primitive and object values.
                <br />
                To create a <span className='font-bold'>global state </span> we need a central store. The store is a higher-order component (HOC) which holds the context (the state) On line 11 we provide an initial default state object and the reducer function to React's useReducer() as arguments.
                <br />
                <span className='font-bold'>Server state</span> or Cache state: The kind of data that is remotely persisted, asynchronous with shared ownership that can be potentially Out of Date.
